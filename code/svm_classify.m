...
weight = zeros(num_categories,size(train_image_feats,2));
offset = zeros(num_categories,1);

for i=1:num_categories
    string = categories(i);
    %create binary labels for each different category string based on
    %matching indices
    matching_indices = strcmp(string, train_labels);
    binary_labels = ones(size(test_image_feats,1),1);
    binary_labels = binary_labels -2;
    
    %add matching indices to binary labels
    binary_labels(matching_indices) = 1;
    
    [weight(i,:), offset(i)] = vl_svmtrain(train_image_feats', binary_labels', LAMBDA);
end
num_test_img = size(test_image_feats,1);

for i = 1 :num_test_img
    conf = zeros(num_categories,1);
    
    for j=1: num_categories
        %confidence of svm that image belongs to class
        conf(j) = sum(weight(j,:) .* test_image_feats(i,:)) + offset(j);
    end
        %maximum confidence of svm is predicted class of image
    [~,ind] = max(conf);
    cats{i} = categories{ind};
end
